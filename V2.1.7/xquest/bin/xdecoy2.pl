...
my ( $verbose,$shuffle,$descstring, $sv, $help, $input, $reverse, $reverse_internal_pep, $targetseq, $outfile, $outpepfile, $dcstring, $shuffleids, $decoy_mode, $binsize, $foreigndb, $params_file );
...
my %PARAMS = (
			   ...
			   'params' => $params_file,
			  # 'enzymenum' => $enzymenum #ABOVE
			  # Maybe the below should all be in a text file
			  # 'correspondingdecoys' => $correspondingdecoys, # takes in a string for the filename FIX THIS
			  # 'missed_cleavages' => $missed_cleavages
			  # 'requiredmissed_cleavages' => $requiredmissed_cleavages
			  # 'tryptic_termini' => $tryptic_termini
			  # 'mindigestlength' => $mindigestlength
			  # 'maxdigestlength' => $maxdigestlength
			  # 'nocutatxlink' => $nocutatxlink
);
## HERE ALL PARAMS THAT COME FROM THE CMD LINE MUST BE DEFINED
GetOptions( \%PARAMS, 'db=s', 'out=s', 'outpep=s', 'decoy=s', 'verbose', 'help', 'targetseq', 'decoystring=s', 'shuffleids', 'binsize=i', 'sverbose', 'foreigndb=s', 'shuffle', 'descstring=s', 'params=s' );
my $PARAMS = \%PARAMS;

if ($PARAMS->{'params'})
{
	_read_params( $PARAMS->{'params'}, $PARAMS )
}
...
&usage() unless ( $decoy_mode eq "cterm" || $decoy_mode eq "nterm" || $decoy_mode eq "ctermpep" || $decoy_mode eq "ntermpep" || $decoy_mode eq "reverse" || $decoy_mode eq "corr" );
my $bndb = $PARAMS->{'db'};
...
unless ($outpepfile)
{
	$outpepfile = $bndb . "_peptide.fasta";
}
$PARAMS->{'db_bn'}=$bndb;
...
my $outpepdb = Bio::SeqIO->new( -file => ">$outpepfile",       -format => 'fasta' );
...
while ( my $seqobj = $indb->next_seq() )
{
	...
	unless( $decoy_mode eq "corr" )
	{
		...
		$ruid++;
	} elsif ( $decoy_mode eq "corr" )
	{
		$cuts = getpeps( $sequence, $enzymenum );
		@scrambledsequences = &{ $scramblefunctions{$decoy_mode} }( $seqobj, $cuts, $PARAMS ); #FIX THIS has no function at the moment
		#print(@scrambledsequences); # FIX THIS
		my $iteration = 0;
		while ( @scrambledsequences[$iteration] ) #->next_seq() ) # FIX THIS does it pull each in order?
		{
			#print @scrambledsequences[$iteration], "\n";
			( $PARAMS->{'sverbose'} ) && print ">$dcstring" . "_$id $descstring of $desc\n";
			$PARAMS->{'sverbose'} && print "@scrambledsequences\n";
			my $newseq = @scrambledsequences[$iteration];
			$sequences->{$ruid}->{'seqobject'} = $newseq;
			$sequences->{$ruid}->{'length'}    = length(@scrambledsequences[$iteration]);
			$iteration++;
			my $newdecoyseq = @scrambledsequences[$iteration];
			$decoysequences->{$ruid}->{'seqobject'} = $newdecoyseq;
			$decoysequences->{$ruid}->{'length'}    = length(@scrambledsequences[$iteration]);
			if ($targetseq)
			{
				print "Adding also traget proteins to the database\n"; # FIX THIS currently uses proteins for targets, need peptides selected
				$sequences->{$ruid}->{'seqobject'} = $seqobj;
				$sequences->{$ruid}->{'length'}    = length($sequence);
				$id++; # FIX THIS confirm that this works, as opposed to ruid?
			}
			$ruid++;
		...
		# FIX THIS
		# sequence is a protein sequence, but that's not okay for the above piece
	}
}	
...
#---------------------------------------------------------------------------
#  Write the resultdatabase if corresponding peptides were generated
#---------------------------------------------------------------------------
if ( $decoy_mode eq "corr" )
{
	foreach my $id ( keys %$sequences )
	{
		my $seqobj = $sequences->{$id}->{'seqobject'};
		$outpepdb->write_seq($seqobj);
		$ruids++;
	}
	
	foreach my $id ( keys %$sequences )
	{
		my $seqobj = $decoysequences->{$id}->{'seqobject'};
		$outdb->write_seq($seqobj);
		$ruids++;
	}
...
sub getpeps
{
	...
	# _digestTryps and others currently use no missed cleavages
	# minimum length, maximum length
	# missed cleavages min, max
	# cross-link sites interior to peptide
...
sub _scramblepep_keep_cterm
{
# FIX THIS not finished yet
	my $seqobj = shift;
	my $cuts = shift;
	my @cutslist = @$cuts;
	my $PARAMS = shift;
	...
	my $minlength = 0;
	my $maxmissedcleavages = 10;
	my $requiredmissedcleavages = 0;
	my $addCuts = 0;
	my $dcstring = "decoy";
	my $missedcleavages = 0;
	my $kindex = 0;
	my @listofseqobjs; # FIX THIS
	
	if( $PARAMS->{'maxdigestlength'} )
	{
		$maxlength = $PARAMS->{'maxdigestlength'}
	}
	if( $PARAMS->{'mindigestlength'} )
	{
		$minlength = $PARAMS->{'mindigestlength'}
	}
	if( $PARAMS->{'missed_cleavages'} )
	{
		$maxmissedcleavages = $PARAMS->{'missed_cleavages'};
	}
	if( $PARAMS->{'requiredmissed_cleavages'} )
	{
		$requiredmissedcleavages = $PARAMS->{'requiredmissed_cleavages'};
	}
	...
	for $i ( 0 .. ($#cutslist - 1) )
	{
		for my $j ( ($i+1) .. ($#cutslist - 1) )
		{
			#print $i," ", $j," ", $#cutslist, "\n";
			#print $cutslist[$i], " ", $cutslist[$j], "\n";
			if ($cutslist[$i] < ($cutslist[$j] - 2) )
			{
				my $seq = $seqobj->subseq( $cutslist[$i], $cutslist[$j] - 1 );
				
				$missedcleavages = ($seq =~ tr/K//) + ($seq =~ tr/R//);
				$kindex = index($seq, "K");
				if( (length($seq) <= $maxlength) && (length($seq) >= $minlength) && ($missedcleavages >= $requiredmissedcleavages) && ($missedcleavages <= $maxmissedcleavages) && ($kindex >= 0) && (($PARAMS->{'nocutatxlink'} == 1 && $kindex <=length($seq)) || ($PARAMS->{'nocutatxlink'} != 1)) )
				{
					#print $missedcleavages," ", $requiredmissedcleavages, " ", $maxmissedcleavages, "\n";
					#print $PARAMS->{'missed_cleavages'}, " ", ($j - $i - $addCuts ), " ", $PARAMS->{'requiredmissed_cleavages'}, " ", ($j - $i - $addCuts), "\n";
					if( ($PARAMS->{'missed_cleavages'} >= ($j - $i - $addCuts )) && ($PARAMS->{'requiredmissed_cleavages'} <= ($j - $i - $addCuts)) )
					{
						my $newpeptideseqobj = Bio::Seq->new(
										-seq  => $seq,
										-desc => $desc,
										-id   => $id,
						);
						...
						foreach my $index (@index)
						{
							$scrambledseq .= $subseq[ $index ];
						}
						$scrambledseq .= $subseq[ length($seq) - 1 ];
						#print $scrambledseq, "\n";
						my $scrambledpeptideseqobj = Bio::Seq->new(
										-seq  => $scrambledseq,
										-desc => $desc,
										-id   => "$dcstring" . "_$id",					
						);
						@listofseqobjs[$counter] = $newpeptideseqobj;
						$counter = $counter + 1;
						@listofseqobjs[$counter] = $scrambledpeptideseqobj;
						$counter = $counter + 1;
					}
				}
				# 'corresponding_decoys' => $correspondingdecoys, # takes in a string for the filename FIX THIS
				# 'missed_cleavages' => $missed_cleavages
				# 'requiredmissed_cleavages' => $requiredmissed_cleavages
				# 'tryptic_termini' => $tryptic_termini
				# 'mindigestlength' => $mindigestlength
	...
	# return a list of peptides (in getpeps)
	# return a list of #scrambled peptides
	# lists should correspond, both need to be printed out
	return @listofseqobjs;
...
sub _digestTryps
{
	...
	while ( $seq =~ /(K|R)/gi )
	{
		...
		{
			# if statement that checks whether the peptide is acceptable due to the parameters
...
sub _digestTrypsForCorr
{
	# FIX THIS copied from Xquest_Digest.pm line #687
	my $seqobj = shift;
	my $ENZ    = shift;
	my $seq    = $seqobj->seq;
	my @seq    = split //, $seq;
...
sub _read_params
{
	...
	open PARAMETERS, "<$params_file" or die "cannot open table $params_file $!";

	while ( my $line = <PARAMETERS> )
	{
		chomp($line);
		if ($line ne "")
		{
