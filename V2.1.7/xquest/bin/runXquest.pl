...
##########################################################
use Environment;
my ( $getdef, $sh, $xmmonly, $runonprottools, $verbose, $help, $xmlfiles, $copydb, $matchlist, $nocompare, $timetolive, $memory, $dev, $listfile, $cponly, $pseudosh, $xmmextracmds, $cp_extracmds, $xq_extracmds, $ssh, $xmlmode, $cpforce, $noxmm, $xmmforce, $brutus, $memsizerequested );
...
GetOptions(
			...
			'timetolive=i',   'xmlmode',        'cpforce',     'noxmm',       'xmmforce',   'brutus',		'euler',	'nocompare','sh',       'getdef'
);
my $PARAMS = \%PARAMS;
...
for ( my $i = 0 ; $i < scalar(@$xq_cmds) ; $i++ )
{
	if ( $PARAMS->{'brutus'} || $PARAMS->{'euler'} )
	{
		...
		{
			$bsuboptions .= "-W " . $PARAMS->{'timetolive'} . ":00" .  " -R \"rusage[mem=$memsizerequested]\"";
		} else
		{
			$bsuboptions .= "-W 4:00" .  " -R \"rusage[mem=$memsizerequested]\"";
